...
export class IndexerGrpcDerivativesStream {
    ...
    streamDerivativeOrderbook({ marketIds, callback, onEndCallback, onStatusCallback, }) {
        const request = InjectiveDerivativeExchangeRpc.StreamOrderbookRequest.create();
        request.marketIds = marketIds;
        const subscription = this.client.StreamOrderbook(request).subscribe({
            next(response) {
                callback(IndexerDerivativeStreamTransformer.orderbookStreamCallback(response));
            },
            error(err) {
                if (onStatusCallback) {
